...
- `templates/`:
  ...
  - `rutina.html` — módulo Rutina (rutina diaria de León + agendas de mamá/papá, horarios en cascada). Columna única mobile-first (max 480px centrada). 7 zonas: topbar sticky propio (header ⏰+fecha+edad / chips multi-select de usuarios) → selector de día L-D → selector de etapa (2m/3m/4m-guardería) → aviso contextual (si no es "hoy real") → tarjetas "Ahora" (solo hoy+etapa actual, con barra de progreso y botón "⏱ Empezó ahora" en la de León) → timeline del día (tap en fila editable = editor inline −15/+15/Ahora/OK; "↺ Plan original" resetea la fecha+etapa) → card de tips. Es puro esqueleto: los contenedores `id="rut-*"` los llena `rutina.js`. Inyecta `window.RUT_DATOS` (solo ajustes persistidos + rango) y carga `rutina-actividades.js` + `rutina.js`. Re-render cada 30 s (reloj vivo + sync de ajustes entre teléfonos). Selección de UI (usuarios/día/etapa) en localStorage `rutina-ui-v1` (por dispositivo); ajustes de horario en SQLite (compartidos).
...
Misma arquitectura IIFE. Fuentes de verdad: `ETAPAS` (constante: definiciones de rutina por etapa, port del design handoff), `window.RUTINA_ACTIVIDADES` (datos), `AJUSTES` = `window.RUT_DATOS.ajustes` (SQLite, shape `fecha → etapa → item_id → min`) y localStorage `rutina-ui-v1` (`{sel, dia, etapa}`, por dispositivo). Fechas SIEMPRE locales con `isoLocal()` — nunca `toISOString()` (UTC corre el día después de las 21:00 ART).

| Función | Propósito |
|---------|-----------|
| `calcular()` | Cascada del día: `run=anchor; start=ajuste??run; end=start+dur` + nocturnas (`inicio noche + off`, dur 25) + `expandir()` de adultos (`link` hereda horario de León y NO es editable — ids derivados `mama-*`/`papa-*` jamás se POSTean; `clock` = minutos absolutos, editable) + cadenas de finde + actividad de estimulación del día (seed por fecha: estable durante el día, rota entre días) |
...
| `resetDia()` | "↺ Plan original": borra la clave local fecha+etapa + POST `/api/rutina/reset` |
| `postAccion(url, campos)` | fetch con `X-Requested-With`; agrega `desde`/`hasta` (semana del cliente); al ok reemplaza `AJUSTES` con el payload fresco SOLO si no hay cambios locales pendientes |
| `syncAjustes()` | Tick de 30 s (+ al volver visible la pestaña): GET `/api/rutina` → trae ajustes del otro teléfono; se saltea si `document.hidden` o hay POSTs pendientes; siempre re-renderiza (reloj vivo) |
| `renderTodo()` | Remide el sticky y re-renderiza las 8 zonas (`renderHeader/Chips/Dias/Etapas/Aviso/Ahora/Timeline/Tips`). El editor inline es estado (`editando`) → sobrevive los re-renders; delegación de eventos en contenedores fijos → el scroll no salta |
| `ajustarSticky()` | `.rut-topbar.style.top` = top computado + alto del `.site-topbar` (cubre el corrimiento del banner DEV). Se llama en cada render, en `resize` y en `load` |
| `fmt(min)` / `fmtDur(dur)` | `HH:MM` módulo 1440 (nocturnas cruzan medianoche) / "45′", "1 h 30′" |

Reglas de vista: tarjetas "Ahora" y resaltado en vivo SOLO si `dia === hoy && etapa === 'actual'`; si no, aviso 👀 contextual. La fila "Sueño nocturno" queda activa desde su inicio hasta las 05:00. Sin toasts: los ajustes son inmediatos y reversibles.
