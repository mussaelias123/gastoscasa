...
4. Validar excepciones → en AJAX retornar `{'ok': False, 'error': str(e)}`. **`ValueError` = validación → 400** (el mensaje es para el usuario y el front lo muestra tal cual); cualquier otra excepción → 500. `/agregar` sigue esa convención; `/editar` atrapa el `ValueError` y vuelve al formulario con el motivo (400).
...
| GET    | `/manifest.json`          | `manifest`            | Manifest de la PWA (`application/manifest+json`). Habilita el botón "Instalar" de Edge/Chrome y `display: standalone` (ventana propia, sin barra de direcciones). Va por ruta y no por `static/` por tres motivos: el `scope` tiene que colgar de la raíz, el Content-Type tiene que ser `application/manifest+json`, y `theme_color`/`background_color` se leen de `cfg.paleta_light` en caliente (siguen a Settings → Paleta). **Fuera del login**: el endpoint `manifest` está en `rutas_publicas` de `auth.py` — el navegador lo pide antes de que haya sesión. Íconos en `static/img/icon-*.png`. El **service worker** es la ruta `/sw.js` de la fila siguiente; el botón de instalar nunca lo necesitó. **Este manifest NO se cachea** y no por olvido: el SW atiende una lista blanca de `/static/`, así que esta ruta —que lee la paleta en caliente— queda afuera por construcción. |
| GET    | `/sw.js`                  | `service_worker`      | Service worker de la PWA (`application/javascript` + `Cache-Control: no-cache` explícito). **El default `sw_enabled=False` sirve la LÁPIDA** (un SW que borra todas las cachés, se desregistra y suelta el control; SIN fetch handler, o sea que la app queda IDÉNTICA). Con `True` sirve el SW real: precachea el esqueleto (`style.css` y `app.js` con el mismo `?v=` que emite `base.html`, más las 4 fuentes sin query) y atiende una **LISTA BLANCA** — solo GET, solo de este origen, solo bajo `/static/`, cache-first contra la URL completa. Las 13 páginas (que se renderizan acá **con los saldos adentro del HTML**), las rutas `/api/*`, el manifest, `/sw.js`, el login y el CSV quedan afuera **por construcción**: el endpoint que se escriba mañana nace protegido solo. ⚠ **No convertir esa lista blanca en lista negra** — `tests/test_sw.py` (clase `TestListaBlanca`) congela las guardas justamente porque esa falla sería silenciosa: un saldo viejo se ve igual que uno fresco. Qué entra y qué no, en `CONTEXT_FRONTEND.md`. Cuelga de la RAÍZ porque un service worker solo controla las URLs bajo su propio path (desde `/static/` no controlaría ninguna página). El cuerpo sale de `templates/sw.js` leído como TEXTO PLANO (**nunca `render_template`**: dispararía todos los context processors —incluido el de notificaciones— en cada navegación, y Jinja se comería los `{{ }}` del JavaScript); el marcador `__VERSION__` se reemplaza a mano por `_static_version()`. **Fuera del login**: endpoint `service_worker` en `rutas_publicas` de `auth.py` — si cayera en el login, el navegador recibiría HTML donde espera JS. Se apaga EN CALIENTE desde `config.json` (`sw_enabled`), sin reiniciar el servicio. |
...
| POST   | `/eliminar/<id>`          | `eliminar`            | Borra movimiento por id. Lee el movimiento ANTES para dos cosas: rechazar con redirect si es personal de la otra persona (`_es_ajeno`) y, si era personal propio, volver a `/personal` en vez de a la tabla del fondo. |
...
| POST   | `/api/lactancia/crear`    | `api_lactancia_crear` | Alta de partida. El destino (`ubicacion`) se elige en el form: heladera o freezer. Valida con `_lac_leer_form_alta` + `_lac_validar_capacidad`. |
| POST   | `/api/lactancia/<id>/cerrar` | `api_lactancia_cerrar` | Marca `usada` \| `descartada` (form `motivo`). `fecha_cierre` vacía → hoy. Form opcional `consumido_ml` (solo `usada`): ml que el bebé realmente tomó, 0..volumen; sin dato se asume que tomó todo. |
| POST   | `/api/lactancia/<id>/bajar` | `api_lactancia_bajar` | Baja una bolsa del freezer a la heladera para descongelar: crea una heladera `tipo='descongelada'` (vence a las N h de bajarla) y cierra la de freezer como `trasladada`. NO bloquea por vencimiento (los avisos son orientativos); `reabrir` sobre la de freezer deshace la bajada. |
| POST   | `/api/lactancia/recordatorio` | `api_lactancia_recordatorio` | Guarda el recordatorio nocturno: form `activo` (`1`/`0`) + `hora` (`HH:MM`). Escribe config y devuelve el payload fresco. |
| POST   | `/api/lactancia/bebe`     | `api_lactancia_bebe`  | Guarda el perfil del bebé: form `nombre` (≤40) + `fecha_nacimiento` (`YYYY-MM-DD`, opcional, no futura). Escribe config y devuelve el payload fresco. |
| POST   | `/api/lactancia/freezar`  | `api_lactancia_freezar` | Combina las heladeras tildadas (form `ids` CSV) en 1 partida de freezer: volumen sumado, extracción más vieja. Una partida VENCIDA se puede freezar igual, pero SOLO con `confirmar_vencidas=1` en el form (la usuaria declara que se pasó al freezer antes de vencerse y se cargó tarde en la app); sin ese flag, 400. Con 2 o más partidas exige que cada una lleve `combinar_min_horas` en heladera (mensaje con cuánto falta, vía `_horas_texto`); con una sola no aplica. Respeta el tope de bolsita. |
| POST   | `/api/lactancia/config`   | `api_lactancia_config` | Guarda el panel "Ajustes" (los 11 parámetros del módulo). **Solo toca los campos que lleguen**, así cada grupo de la pantalla se guarda por su cuenta. Valida rangos contra `config.LIMITES_LACTANCIA` + tres reglas de coherencia: ningún aviso puede ser mayor que el vencimiento que avisa. |
| GET    | `/descargar/dia-a-dia.csv`| `descargar_dia_a_dia` | Descarga la tabla día a día (`_lac_dia_a_dia`). BOM + separador `;` + fechas d/m/Y para que Excel en castellano la abra bien de una. |
...
| POST   | `/api/rutina/soltar`      | `api_rutina_soltar`   | Borra el ajuste de UN ítem ("Soltar" del popover). Form `fecha`, `etapa`, `item_id`. En la rutina generada del bebé esa fila es lo que lo clava: sin ella vuelve a encadenarse. No toca la duración. |
...
| POST   | `/api/rutina/pendiente/crear` | `api_rutina_pendiente_crear` | Alta de **TAREA** (la lista que se tilda, tabla `rutina_pendientes`). Form `titulo` (1..60), `dibujo`, `repite` (0/1), `dias` (7 bits, solo si `repite=1`), `fecha` (solo si `repite=0`; vacío → hoy), `responsables` (ids separados por coma; puede ir vacío). La fecha de ALTA la pone el servidor: el cliente no puede mandarla como `desde` (ver el aviso de abajo). |
| POST   | `/api/rutina/pendiente/editar` | `api_rutina_pendiente_editar` | Edición (pide `id` + los campos de arriba + `activo`). No toca la fecha de alta. |
| POST   | `/api/rutina/pendiente/borrar` | `api_rutina_pendiente_borrar` | Baja definitiva (form `id`); se lleva los responsables y todo el historial de tildados. |
| POST   | `/api/rutina/pendiente/marcar` | `api_rutina_pendiente_marcar` | Tilda/destilda. Form `id`, `fechas` (una o dos, separadas por coma: un toque puede cerrar la ocurrencia de ayer arrastrada y la de hoy), `hecha` (0/1). |
...
calculan acá (NUNCA se almacenan: cambiar un parámetro en Ajustes recalcula todo
...
**Camino de la leche**: en el alta se ELIGE el destino (heladera o freezer; por
defecto heladera) y el vencimiento se calcula según ese destino. Además sigue
existiendo el paso de heladera a freezer por combinación de partidas tildadas
(`/api/lactancia/freezar`).

> La pantalla del módulo (template, CSS y JS) se trajo entera de la app suelta de
> banco de leche (`C:\Proyectos\lactancia`, publicada en
> `paralasmamas.pythonanywhere.com`). Lo que NO se trajo: el cambio
> español/inglés, las sugerencias por correo, la PWA y el sistema de cuentas
> propio. Las dos apps comparten las mismas 14 columnas de `lactancia_partidas`.

- `LAC_UBICACIONES = ('freezer', 'heladera')`; `LAC_MOTIVOS_CIERRE = ('usada', 'descartada', 'trasladada')`.
- `LAC_PARAMS_NUM` / `LAC_PARAMS_BOOL` → tuplas `(clave_corta, clave_config)` con los parámetros del panel Ajustes. Separadas porque castean distinto: en los numéricos un `0` es un valor válido (`combinar_min_horas = 0` significa "no me lo verifiques"), así que nunca se decide por verdadero/falso.
- `_lac_params(cfg=None)` → dict con los 11 parámetros de config con claves cortas (los 9 numéricos de `LAC_PARAMS_NUM` casteados a int + los 2 booleanos); fallback a DEFAULTS si vienen corruptos.
- `_lac_validar_capacidad(volumen_ml, params)` → lanza `ValueError` si la capacidad de bolsita está activada y el volumen no entra. La usan `crear` y `freezar`. Es un tope declarado por la usuaria, no una regla sanitaria: apagado por defecto.
- `_horas_texto(horas)` → `'40 min'` / `'2 h'` / `'2 h 20 min'`. Para el aviso de cuánto le falta a una partida para poder combinarse.
- `_lac_dia_de_vida(nac, dia)` / `_lac_dia_a_dia(hoy=None)` → base de la descarga CSV. Un renglón por cada día de vida (el día del parto es día 1 y mes 1, como en pediatría). "Extraído" cuenta SOLO la leche `fresca`: una combinada o una bajada es la misma leche cambiando de lugar, contarla otra vez la duplicaría.
- `_lac_muestras(partidas=None, bebe=None)` → una fila por EXTRACCIÓN REAL: `{id, fecha, hora|None, ml, dia_vida|None, mes_vida|None, dia_semana}` (0=lunes). Materia prima del gráfico "Explorar mis datos" del Resumen: con esa lista el navegador cruza cualquier par de variables sin volver a pedir nada (cambiar de eje = reagrupar en el cliente). MISMO criterio de `_lac_dia_a_dia`: solo `tipo` fresca (`(p.get('tipo') or 'fresca')`, así las filas viejas sin tipo no quedan afuera) — contar la combinada o la descongelada inflaría la producción. Las usadas y descartadas SÍ cuentan (se mira lo producido, no lo que queda). Ambos parámetros son opcionales para poder testearla pura; `_lac_payload()` le pasa lo que ya tiene y evita dos consultas. Tests: `tests/test_lactancia_muestras.py`.
- `_lac_consumos(partidas=None, bebe=None)` → una fila por BOLSITA QUE TOMÓ León: `{id, fecha, hora=None, ml, en_jardin, dia_vida|None, mes_vida|None, dia_semana}`. La segunda fuente del gráfico, con los MISMOS nombres de campo que `_lac_muestras` a propósito: así los ejes horizontales sirven para las dos sin una línea extra. Solo `motivo_cierre == 'usada'` (una descartada se tiró, una trasladada cambió de lugar). La fecha es la del **cierre**, no la de extracción — la pregunta es cuándo TOMÓ; de ahí salen también `dia_semana` y la edad. `hora` siempre `None`: el cierre guarda el día y nada más, y la pantalla usa eso para apagar los ejes por hora. `en_jardin` es lo que separa lo tomado en el jardín de lo tomado en casa. Al revés que en `_lac_muestras`, NO filtra por `tipo`: una bolsita se usa una sola vez. Tests: `tests/test_lactancia_consumos.py`.
- `_lac_consumido(p)` → ml que tomó de una bolsita 'usada': `consumido_ml` si se anotó, si no el volumen entero. Regla compartida por el KPI del tablero, `_lac_consumos` y `_lac_dia_a_dia` — estaba escrita tres veces, ahora vive acá.
- `_lac_extraccion_dt(p)` → `datetime` del momento real de extracción (`fecha_extraccion` + `hora_extraccion`; sin hora — solo legacy — cae a las 00:00 de ese día, fallback conservador). Espera dict.
- `_lac_vencimiento(p, params)` → `datetime`, desde la extracción real (issue #48; `cargada` = solo auditoría). Freezer: extracción + N meses (reusa `_act_sumar_intervalo`, clamp fin de mes) al fin del día 23:59:59 (usable el día que vence, igual que el Excel). Heladera: `_lac_extraccion_dt` + N horas — cruza medianoche sin caso especial. **Excepción `tipo='descongelada'`**: `cargada` + `descongelada_horas` — la leche bajada del freezer corre desde que se baja, no desde una extracción que puede ser de hace meses.
- `_lac_estado(p, params, ahora)` → cascada: `motivo_cierre` → ese estado > `vencida` > `vence_pronto` (ventana: freezer en días, heladera en horas; la `descongelada` usa `aviso_descongelada_horas`) > `disponible` | `en_heladera`.
- `_lac_horas_en_heladera(p, ahora)` / `_lac_freezable(p, params, ahora)` / `_lac_freezar_reciente(p, params, ahora)` → antigüedad en heladera (desde la EXTRACCIÓN; la `descongelada` desde `cargada`); si la partida TODAVÍA puede pasar al freezer (abierta, no vencida y **NO `descongelada`** — lo que ya estuvo congelado no se vuelve a congelar); y si lleva menos de `freezar_hasta_horas` (solo define el tildado por defecto del checkbox, no bloquea nada). `_lac_freezable` espera dict (no `sqlite3.Row`).
- **Recordatorio nocturno de bajar bolsitas** (aviso in-app, nunca bloquea): `_lac_recordatorio(cfg=None)` → `{activo, hora}` desde config (fallback a DEFAULTS si la hora viene corrupta). `_lac_bajo_leche_hoy(ahora)` → True si ya hay una partida `descongelada` con `cargada` de hoy (así el recordatorio se autolimpia al bajar algo). `_lac_recordatorio_pendiente(rec=None, ahora=None)` → True si: activo + ya pasó la hora de hoy + hay freezer ABIERTO para bajar + todavía no se bajó ninguna hoy. Lo consumen el provider de notificaciones y `_lac_payload()`.
- `_lac_bebe(cfg=None, ahora=None)` → `{nombre, fecha_nacimiento, edad_texto, mes_de_vida}`. `edad_texto` en meses **y días** ("2 meses y 9 días"; días sueltos vía `_act_sumar_intervalo` para clampear fin de mes); nombre vacío → `"el bebé"`. A propósito NO calcula nada médico (peso/estatura/cuánta leche "debería" tomar).
- `_lac_enriquecer(row, params, ahora)` → dict (incluye `tipo` y `consumido_ml`) + `vencimiento` (ISO), `estado`, `dias_restantes` (freezer) / `horas_restantes` + `horas_en_heladera` + `freezable` + `freezar_reciente` (heladera).
- `_lac_payload()` → `{'freezer': [FIFO], 'heladera': [FIFO], 'historial': [cerradas DESC], 'tablero': {...}, 'params': {...}, 'badge': int, 'recordatorio': {activo,hora,pendiente}, 'bebe': {nombre,fecha_nacimiento,edad_texto,mes_de_vida}, 'muestras': [_lac_muestras], 'consumos': [_lac_consumos]}`. FIFO = vencimiento asc, desempate por hora de extracción y luego id. Tablero: usables = disponible+vence_pronto (vencidas NO suman; la del jardín es leche sana de León, solo guardada allá, y como el jardín es una marca y no un estado ya viene contada acá); trasladadas no cuentan como usadas ni descartadas; heladera separada del freezer. Fuente de TODAS las respuestas AJAX del módulo.
  - **KPIs de ciclo de vida en `tablero`**: `producido_ml` (Σ volumen de las `fresca` — cada extracción UNA vez, ver CONTEXT_DB), `descongelada_ml`, `consumida_ml` (Σ `consumido_ml`, o el volumen si no se anotó) con su desglose `consumida_jardin_ml` / `consumida_fuera_ml` (por la marca `en_jardin` que quedó congelada al cerrar la bolsita; los dos suman siempre el total, y lo usado antes de que la marca existiera cae en "fuera"), `desperdicio_ml` (descartado + lo que sobró en usadas con consumo anotado), `heladera_fresca_ml` / `heladera_descongelada_ml`, y dos de **promedio móvil**: `bolsa_sugerida_ml` (promedio del consumo real anotado) y `dias_stock` (stock usable **de freezer + heladera** — el mismo de `stock_total_ml` — ÷ consumo diario de los últimos 7 días). Los dos últimos devuelven `None` cuando todavía no hay datos — el front muestra "—". **Trampa de la ventana de 7 días**: el consumo semanal es un generador con filtro, así que esa línea SOLO se ejecuta si hay alguna bolsita usada dentro de la ventana. Una base sin usadas recientes (dev) nunca la corre y una con usadas de esta semana (casa) sí — fue el 500 del 2026-09-09, que volteó TODAS las páginas porque el Inicio también pide este payload. Tests: `tests/test_lactancia_tablero.py`, con fechas relativas a hoy a propósito.
- `_home_lactancia_payload()` → `{'heladera': [...], 'freezer_primera': dict|None, 'params': {...}}`. Proyección de `_lac_payload()` para la tarjeta Lactancia del Inicio (todas las de heladera + la primera del freezer, FIFO). Solo recorta — JAMÁS reimplementa vencimientos/estados. La consume `index()` (`lac_home` → `window.LAC_HOME`). `params` viaja porque el form de alta del Inicio es el MISMO partial que el de `/lactancia`: lo necesita para pintar el vencimiento en vivo mientras se carga.
...
- `_rut_payload(desde, hasta)` → `{'ajustes': {fecha: {etapa: {item_id: min}}}, 'duraciones': {misma forma, dur_min}, 'miembros': [...], 'actividades': [...], 'config': {...}, 'hoy', 'desde', 'hasta', 'tareas': [...], 'pendientes': [...], 'pendientes_hechas': [...], 'ocultos': [...], 'calendario': [...]}`. Fuente de TODAS las respuestas AJAX del módulo. ⚠ `tareas` y `pendientes` son cosas DISTINTAS que la UI llama igual: `tareas` = `rutina_tareas`, las sueltas CON horario de la línea de tiempo; `pendientes` = `rutina_pendientes`, la lista de Tareas que se tilda. ⚠ `pendientes_hechas` arranca **un día antes de `desde`**: una tarea que no se hizo ayer se arrastra a hoy, así que para saber si hay que mostrarla hace falta el estado de ayer — y el Inicio pide el payload con un rango de UN día. `calendario` = actividades del módulo Calendario (no terminadas) cuya `_act_proxima_fecha()` es HOY, como `{id, nombre, responsable}` — alimenta la tarjeta "Hoy por calendario" de /rutina (ofrece añadirlas a la rutina; NO toca el estado del Calendario).
- `_rut_miembro_id(valor, campo)` → valida contra los miembros activos. Reemplaza a la vieja tupla fija `('leon','mama','papa')`.
- `_rut_ids_lista(valor, campo, excluir=None, tope=8)` → `'3,7'` → `[3, 7]`. Lista de ids de miembros activos, sin repetidos y sin `excluir`; `''` → `[]`. Una sola query para todos (a diferencia de `_rut_miembro_id`, que valida uno). La usan `participantes` de una actividad y `acompanan` de un bebé.
...
- `_rut_leer_form_miembro(form)` → `(nombre, rol, es_bebe, fecha_nacimiento, dibujo, color_token, ancla_min, acompanan)`. Nombre 1..40; `es_bebe` solo con `rol='hijo'`; fecha no futura y **obligatoria si es bebé** (sin edad no hay ventana de sueño); `ancla_min` 0..1439. `acompanan` se **fuerza a `''` si `es_bebe=0`** (si no, destildar "es bebé" dejaría un dato fantasma que volvería a aplicarse al re-tildarla); se excluye el propio id y el tope es 3.
...
- `_rut_leer_form_pendiente(form)` → `(titulo, dibujo, repite, dias, fecha, responsables)`. Título 1..60. Bifurca ANTES de `_rut_bits`: con `repite=0` no lo llama y deja `dias='0000000'` (si lo llamara, el todo-ceros reventaría). `responsables` usa `_rut_ids_lista` **sin `excluir`** — una tarea no tiene dueño — y la lista vacía es válida (tarea de la casa).
- `_rut_fechas_lista(valor, campo, tope=3)` → `'2026-09-16,2026-09-17'` → lista de fechas válidas, sin repetidos y **ninguna posterior a mañana** (el reloj del teléfono puede estar corrido, pero de ahí a dejar cerrar febrero entero hay un trecho).
- `_rut_barrer_pendientes(hoy)` → corre `cerrar_pendientes_vencidas()` **una vez por día por proceso** (marcador en el global `_RUT_BARRIDO`; NSSM levanta un solo proceso). Lo llama `_rut_payload()`. ⚠ Es una ESCRITURA en un camino de lectura: por eso es idempotente, está acotado por `lookback` y va en `try/except` que nunca propaga — si el barrido falla, la rutina se tiene que ver igual.
- `_rut_leer_form_actividad(form)` → `(miembro_id, titulo, dibujo, inicio_min, dur_min, dias, meses, desde, hasta, anual, nota, participantes)`. Título 1..60, `inicio_min` 0..1439, `dur_min` 5..720, `nota` ≤200. El dueño se descarta de `participantes` (si no, la actividad saldría dos veces en su propia columna).
