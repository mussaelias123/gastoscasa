...
- `_lac_vencimiento(p, params)` → `datetime`, SIEMPRE desde la extracción real (issue #48; `cargada` = solo auditoría). Freezer: extracción + N meses (reusa `_act_sumar_intervalo`, clamp fin de mes) al fin del día 23:59:59 (usable el día que vence, igual que el Excel). Heladera: `_lac_extraccion_dt` + N horas — cruza medianoche sin caso especial.
...
- `_lac_horas_en_heladera(p, ahora)` / `_lac_freezable(p, params, ahora)` / `_lac_freezar_reciente(p, params, ahora)` → antigüedad en heladera (desde la EXTRACCIÓN, mismo criterio que el vencimiento); si la partida TODAVÍA puede pasar al freezer (abierta, no vencida — sin tope de horas, el usuario decide); y si lleva menos de `freezar_hasta_horas` (solo define el tildado por defecto del checkbox, no bloquea nada). `_lac_freezable` espera dict (no `sqlite3.Row`).
