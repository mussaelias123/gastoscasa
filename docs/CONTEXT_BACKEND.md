...
| GET    | `/manifest.json`          | `manifest`            | Manifest de la PWA (`application/manifest+json`). Habilita el botón "Instalar" de Edge/Chrome y `display: standalone` (ventana propia, sin barra de direcciones). Va por ruta y no por `static/` por tres motivos: el `scope` tiene que colgar de la raíz, el Content-Type tiene que ser `application/manifest+json`, y `theme_color`/`background_color` se leen de `cfg.paleta_light` en caliente (siguen a Settings → Paleta). **Fuera del login**: el endpoint `manifest` está en `rutas_publicas` de `auth.py` — el navegador lo pide antes de que haya sesión. Íconos en `static/img/icon-*.png`. El **service worker** es la ruta `/sw.js` de la fila siguiente (hoy inerte); el botón de instalar nunca lo necesitó. Cuando se le sume caché de verdad, va network-first y sin cachear `/api/*`. |
| GET    | `/sw.js`                  | `service_worker`      | Service worker de la PWA (`application/javascript` + `Cache-Control: no-cache` explícito). **Hoy INERTE**: el default `sw_enabled=False` sirve la LÁPIDA (un SW que borra todas las cachés, se desregistra y suelta el control) y con `True` sirve el SW real, que en esta etapa todavía está vacío — en los dos casos SIN fetch handler, así que la app queda idéntica. Cuelga de la RAÍZ porque un service worker solo controla las URLs bajo su propio path (desde `/static/` no controlaría ninguna página). El cuerpo sale de `templates/sw.js` leído como TEXTO PLANO (**nunca `render_template`**: dispararía todos los context processors —incluido el de notificaciones— en cada navegación, y Jinja se comería los `{{ }}` del JavaScript); el marcador `__VERSION__` se reemplaza a mano por `_static_version()`. **Fuera del login**: endpoint `service_worker` en `rutas_publicas` de `auth.py` — si cayera en el login, el navegador recibiría HTML donde espera JS. Se apaga EN CALIENTE desde `config.json` (`sw_enabled`), sin reiniciar el servicio. |
