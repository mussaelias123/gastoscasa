...
- `_lac_params(cfg=None)` → dict con los 7 parámetros de config casteados a int (claves cortas: `freezer_meses`, `heladera_horas`, `descongelada_horas`, `aviso_freezer_dias`, `aviso_heladera_horas`, `aviso_descongelada_horas`, `freezar_hasta_horas`); fallback a DEFAULTS si vienen corruptos.
