...
| Push (suscripciones) | `database.py` (tabla `push_suscripciones` + `guardar_suscripcion_push` / `borrar_suscripcion_push` / `borrar_suscripciones_push_de_email` / `obtener_suscripciones_push`), `app.py` (`/api/push/alta`, `/api/push/baja`, `_push_leer_suscripcion`), `auth.py` (el `logout` que limpia), `tests/test_push_suscripciones.py`. ⚠ **Acá NO se manda ningún push**: solo se guarda a quién avisarle. `push_enabled` sigue apagado y sin que nadie lo lea | `docs/CONTEXT_DB.md` (la tabla y el porqué del `WHERE` del upsert) + `docs/CONTEXT_BACKEND.md` (las dos rutas) + `docs/CONTEXT_AUTH.md` (el logout) |
