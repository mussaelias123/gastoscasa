...
| Push (envío) | `app.py` (`/api/push/prueba` + `_push_payload` / `_push_enviar` / `_PUSH_TIMEOUT` / `_PushSinClaves`), `templates/sw.js` (listeners `push` y `notificationclick` en la mitad ACTIVO), `static/app.js` (`window.Push`), `templates/settings.html` (tarjeta "Avisos en el teléfono"), `auth.py` (el logout que ahora borra SOLO el navegador que se va), `tests/test_push_envio.py`. ⚠ **El payload nunca lleva montos ni saldos** (se lee en la pantalla bloqueada) y la `url` va RELATIVA. Lo único que MANDA un push sigue siendo un BOTÓN, y `push_enabled` sigue apagado | `docs/CONTEXT_PUSH.md` (envío) + `docs/CONTEXT_BACKEND.md` (la ruta) + `docs/CONTEXT_FRONTEND.md` (la tarjeta) |
| Push (scheduler EN SECO) | `app.py` (registry `PUSH_AVISOS` justo después de `NOTIF_PROVIDERS`, + `_push_avisos_ahora` / `_push_estado_leer` / `_push_estado_guardar` / `_push_ciclo` / `_scheduler_push` / `iniciar_scheduler_push` / `_PUSH_INTERVALO`), `tests/test_push_scheduler.py`, `TempScripts/simular_flancos_push.py`. Estado en `push_estado.json` (dentro de `backup_dir`, **no en la DB**). ⚠ **Ya hay un hilo que corre solo, pero NO manda nada**: detecta el FLANCO ascendente y loguea `SECO: ...` lo que mandaría. No llama a `_push_enviar()` ni lee las suscripciones. El `detalle` del ítem NO entra en la clave de dedup ni en el cuerpo | `docs/CONTEXT_PUSH.md` (§5 el scheduler) + `docs/CONTEXT_BACKEND.md` (helpers y schedulers) + `docs/CONTEXT_DEPLOY.md` (cómo se lee el ensayo) |
...
| Notificaciones (sumar provider de un módulo, campana, panel) | `CLAUDE.md` + `CONTEXT_NOTIFICATIONS.md` (+ `CONTEXT_BACKEND.md` o `CONTEXT_FRONTEND.md` según la capa que toques). ⚠ Sumar un provider a `NOTIF_PROVIDERS` **no** lo hace mandar pushes: eso se decide aparte, en `PUSH_AVISOS` (ver `CONTEXT_PUSH.md`) | según capa |
...
| PWA — push (avisos automáticos, scheduler, payload) | `CLAUDE.md` + `CONTEXT_PUSH.md` (**todo el dominio**: payload, envío, el scheduler en seco) + `CONTEXT_DEPLOY.md` si mirás el ensayo en el log | según capa |
