...
| Notificaciones (sumar provider de un módulo, campana, panel) | `CLAUDE.md` + `CONTEXT_NOTIFICATIONS.md` (+ `CONTEXT_BACKEND.md` o `CONTEXT_FRONTEND.md` según la capa que toques). ⚠ Sumar un provider a `NOTIF_PROVIDERS` **no** lo hace mandar pushes: eso se decide aparte, en `PUSH_AVISOS` (ver `CONTEXT_PUSH_MOTOR.md`) | según capa |
